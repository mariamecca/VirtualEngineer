          ...
          {project.budget > 0 && (
            <div className="card">
              ...
              <div className="grid grid-cols-3 gap-4 text-center">
                <div>
                  <p className="text-gray-400 text-xs uppercase tracking-wide">Totale</p>
                  <p className="text-xl font-bold text-white mt-1">€{project.budget.toLocaleString()}</p>
                </div>
                <div>
                  <p className="text-gray-400 text-xs uppercase tracking-wide">Consumato stimato</p>
                  <p className="text-xl font-bold text-amber-400 mt-1">
                    €{Math.round(project.budget * (project.progress || 0) / 100).toLocaleString()}
                  </p>
                </div>
                <div>
                  <p className="text-gray-400 text-xs uppercase tracking-wide">Residuo stimato</p>
                  <p className="text-xl font-bold text-green-400 mt-1">
                    €{Math.round(project.budget * (100 - (project.progress || 0)) / 100).toLocaleString()}
                  </p>
                </div>
              </div>
              <div className="mt-3 h-2 bg-gray-800 rounded-full overflow-hidden">
                <div className="h-full bg-amber-500 rounded-full transition-all" style={{ width: `${project.progress || 0}%` }} />
              </div>
              <p className="text-gray-600 text-xs mt-1">Stima basata sull'avanzamento attuale</p>
            </div>
          )}
